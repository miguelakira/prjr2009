...
class UploadBehavior extends ModelBehavior {
    ...
    if (!empty($this->toWrite)) {
      ...
      foreach ($this->toWrite as $field => $toWrite) {
        ...
        if (is_dir($destDir) && is_writable($destDir)) {
          if (@move_uploaded_file($toWrite['tmp_name'], $settings['path'].DS.$toWrite['name'])) {
            foreach ($settings['styles'] as $style => $geometry) {
              $newSettings = $this->_interpolate($model, $field, $toWrite['name'].DS.$toWrite['name'], $style);
              $this->_resize($settings['path'].DS.$toWrite['name'], $newSettings['path'].DS.$toWrite['name'], $geometry);
            }
          }
        }
      }
  ...
  function _resize($srcFile, $destFile, $geometry) {
    ...
    if ($src = $createHandler($destFile)) {
      ...
      imagecopyresampled($img, $src, 0, 0, 0, 0, $destW, $destH, $srcW, $srcH);
